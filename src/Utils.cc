...
using namespace std;
static time_t start_timer, stop_timer, phil_timer, day_timer;
static char ErrorFilename[PHIL_STRING_SIZE];

void Utils::phil_abort(const char* format, ...) {
    ...
    if (Global::ErrorLogfp == NULL) {
        ...
        if (Global::ErrorLogfp == NULL) {
            // output to stdout
            printf("PHIL ERROR: Can't open errorfile %s\n", ErrorFilename);
    ...
    va_list ap;
    va_start(ap,format);
    fprintf(Global::ErrorLogfp,"PHIL ERROR: ");
...
void Utils::phil_warning(const char* format, ...) {
    ...
    va_list ap;
    va_start(ap,format);
    fprintf(Global::ErrorLogfp,"PHIL WARNING: ");
    vfprintf(Global::ErrorLogfp,format,ap);
    va_end(ap);
    fflush(Global::ErrorLogfp);
...
void Utils::phil_open_output_files(char * directory, int run) {
    ...
    if (strcmp(Global::Immunityfilebase, "none") != 0) {
        ...
        if (Global::Immunityfp == NULL) {
            Utils::phil_abort("Help! Can't open %s\n", filename);
        }
...
void Utils::phil_make_directory(char * directory) {
    ...
    if (0!=mkdir(directory, mode) && EEXIST!=errno) // make it
        Utils::phil_abort("mkdir(%s) failed with %d\n", directory, errno); // or die
}


void Utils::phil_end(void) {
    // This is a function that cleans up PHIL and exits
...
void Utils::phil_print_wall_time(const char* format, ...) {
    time_t clock;
    time(&clock);
    va_list ap;
    va_start(ap,format);
...
void Utils::phil_start_timer() {
    time(&phil_timer);
    start_timer = phil_timer;
}

void Utils::phil_start_timer(time_t * lap_start_time) {
    time(lap_start_time);
}

void Utils::phil_start_day_timer() {
    time(&day_timer);
    start_timer = day_timer;
}

void Utils::phil_print_day_timer(int day) {
    time(&stop_timer);
...
void Utils::phil_print_finish_timer() {
    time(&stop_timer);
    fprintf(Global::Statusfp, "PHIL took %d seconds\n",
            (int)(stop_timer-phil_timer));
    fflush(Global::Statusfp);
}

void Utils::phil_print_lap_time(const char* format, ...) {
    time(&stop_timer);
    va_list ap;
    va_start(ap,format);
    vfprintf(Global::Statusfp,format,ap);
...
void Utils::phil_print_lap_time(time_t * start_lap_time, const char* format, ...) {
...
void Utils::phil_verbose(int verbosity, const char* format, ...) {
...
void Utils::phil_verbose_statusfp(int verbosity, const char* format, ...) {
    if (Global::Verbose > verbosity) {
        va_list ap;
        va_start(ap,format);
        vfprintf(Global::Statusfp,format,ap);
...
void Utils::phil_log(const char* format, ...) {
    va_list ap;
    va_start(ap,format);
    vfprintf(Global::Statusfp,format,ap);
    va_end(ap);
...
void Utils::phil_report(const char* format, ...) {
    va_list ap;
    va_start(ap,format);
...
FILE *Utils::phil_open_file(char * filename) {
    FILE *fp;
    get_phil_file_name(filename);
    printf("phil_open_file: opening file %s for reading\n", filename);
    fp = fopen(filename, "r");
    return fp;
}

void Utils::get_phil_file_name(char * filename) {
    string str;
    str.assign(filename);
    if (str.compare(0,10,"$PHIL_HOME") == 0) {
        char * phil_home = getenv("PHIL_HOME");
        if (phil_home != NULL) {
            str.erase(0,10);
            str.insert(0,phil_home);
            strcpy(filename, str.c_str());
        } else {
            phil_abort("get_phil_file_name: the PHIL_HOME environmental variable cannot be found\n");
        }
    }
}
...
void Utils::phil_print_resource_usage(int day) {
    rusage r_usage;
