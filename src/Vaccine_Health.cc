...
int Vaccine_Health::num_remaining_doses() {
    int last_dose = vaccine->get_number_doses() - 1;
    return last_dose - current_dose;
}

bool Vaccine_Health::more_updates_needed(int day) {
    if (num_remaining_doses()==0 && vaccination_effective_day <= day) {
        return false;
    }
    else {
        return true;
...
void Vaccine_Health::update(int day, int age) {
    ...
    if (num_remaining_doses() > 0) {   // Are we done with the dosage?
