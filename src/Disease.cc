...
Disease::Disease() {
    ...
    immunity_loss_mu = -1.0;
    immunity_loss_sigma = -1.0;
    mutation_prob = NULL;
...
void Disease::setup(int disease, Population *pop, double *mut_prob) {
    ...
    Params::get_indexed_param("immunity_loss_mu",id,&immunity_loss_mu);
    Params::get_indexed_param("immunity_loss_sigma",id,&immunity_loss_sigma);
...
void Disease::add_root_strain(int num_elements) {
    strain_table->add_root_strain(num_elements);
}
